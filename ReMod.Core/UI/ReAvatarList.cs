...
namespace ReMod.Core.UI
{
    ...
    public class ReAvatarList : UiElement
    {
        ...
        public ReAvatarList(string title, IAvatarListOwner owner,Transform parent, bool addClearButton = true, bool addPagination = true) : base(
            LegacyAvatarList,
            parent,
            $"{title}AvatarList",
            false)
        {
            ...
            EnableDisableListener1.RegisterSafe();
            var enableDisableListener = GameObject.AddComponent<EnableDisableListener1>();
        ...
        public void Refresh(AvatarList avatars)
        {
            if (_hasPagination)
            {
                ...
                AvatarList cutDown = avatars.GetRange(_currentPage * _maxAvatarsPerPage,
